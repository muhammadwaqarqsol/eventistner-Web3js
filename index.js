...
const startEventListener = async (chain, wssEndpoint, contractAddress) => {
  const provider = new WebSocketProvider(wssEndpoint, {
    clientConfig: {
      // WebSocket client options
      keepalive: true,
      keepaliveInterval: 60000, // Adjust as needed
    },
    reconnect: {
      auto: true,
      delay: 5000, // Initial reconnect delay (ms)
      maxAttempts: 10, // Maximum reconnect attempts
    },
  });
...
const subscribeToEvent = async (chain, contract, eventName) => {
  ...
  subscription.on('connected', subscriptionId => {
    console.log(`${chain} ERC20 '${eventName}' SubID:`, subscriptionId);
  });

  subscription.on('data', event => {
    console.log(`${chain} ERC20 '${eventName}'`, JSON.stringify({ event }));
    console.log(
          `${chain} ERC20 '${eventName}' Transaction Hash:`,
  ...
  subscription.on('changed', event => {
    // Handle event changes or cleanup
  });

  subscription.on('error', error => {
    console.error(`${chain} ERC20 '${eventName}' error:`, error);
  });
};
